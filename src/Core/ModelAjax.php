...
use Marve\Ela\MySql\QueryAjax;
use Marve\Ela\Validation\Validator;
use stdClass;

abstract class ModelAjax extends QueryAjax implements DatabaseInterface, ListInterface
{
    ...
    public function remove(string $id, string $column, int $user = 0)
    {
         if(CurrentUser::isUser())
         {
            ...
            if($id == 0)
                $this->message = $this->conexion->error;
            return $id;
         }
         else return 0;
    }
    ...
    public function edit(stdClass $data, string $id, string $column = '', int $user = 0)
    {
         if(CurrentUser::isUser())
        {
            ...
            {               
                ...
                {
                    $data->$key = $this->conexion->real_escape_string($validar);
                }
